"""
Direct testing script for all 10 vulnerability types.
Bypasses frontend and Celery - directly calls LLM generator.

Usage (from project root):
    python backend/scripts/test_all_vulns_direct.py
"""
...
def main():
    ...
    print(f"\n✓ Successful: {len(successful)}/{len(VULN_TYPES)}")
    for v in successful:
        print(f"  - {v}")

    if failed:
        print(f"\n✗ Failed: {len(failed)}/{len(VULN_TYPES)}")
        for v in failed:
            print(f"  - {v}")
